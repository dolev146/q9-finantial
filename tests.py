...
class TestBudgetDecomposition(unittest.TestCase):
    ...
    def test_example_scenario(self):
      ...
      expected = [[100.0, 0.0, 0, 0],[100.0, 0, 0.0, 0],[100.0, 0, 0, 0],[0, 50.0, 50.0, 0],[100.0, 0, 0, 0]]
    ...
    def test_example2_scenario(self):
        budget = [100, 200]
        preferences = [{0}, {1}, {1}]
        expected = [[100.0, 0], [0, 100.0], [0, 100.0]]
        result = find_decomposition(budget, preferences)
        self.assertEqual(result, expected)

    def test_example3_scenario(self):
        budget = [0, 300]
        preferences = [{0}, {1}, {1}]
        expected = None
        result = find_decomposition(budget, preferences)
    ...
    def test_excess_budget(self):
        budget = [300, 600]
        preferences = [{0}, {1}]
        expected = [[300.0, 0], [0, 600.0]]
        result = find_decomposition(budget, preferences)
        self.assertEqual(result, expected)



    def test_all_decompositions_are_true_budget(self):
        budget = [50, 100]
        preferences = [{0, 1}, {0, 1}, {0, 1}]
        result = find_decomposition(budget, preferences)
        expected = [[50.0, 0.0], [0, 50.0], [0, 50.0]]
        self.assertEqual(result, expected)


    def test_no_preferences_for_subject_with_budget(self):
        budget = [100, 200]
        preferences = [{}, {0,1}]
        result = find_decomposition(budget, preferences) # edge case , here I need to fix there is no decomposition.
